...
docker build -t library-checker-images-clang -f $SCRIPT_DIR/Dockerfile.CLANG $SCRIPT_DIR
...
docker build -t library-checker-images-scheme -f $SCRIPT_DIR/Dockerfile.SCHEME $SCRIPT_DIR
